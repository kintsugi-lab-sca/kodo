...
- [~] **ORCH-01** *(código ✅ verificado 9/9 · HUMAN-UAT falló 2026-06-24 → superado por ORCH-02)*: El orquestador propone proactivamente adoptar una sesión ad-hoc y deriva un **título inteligente** del contexto real (cwd / commits / transcript), que pasa por el sanitizador del núcleo (BIDIR-08) y se confirma (humano/CLI) antes de crear. Implementado shelleando el mismo `kodo adopt --title "<derived>"` — el carril 0-token del núcleo se preserva, el LLM vive estrictamente en el consumidor (prosa del skill `kodo-orchestrate` actualizada; cero lógica de negocio nueva en el orquestador). **Hallazgo UAT (2026-06-24):** el orquestador NO puede resolver `workspace_ref`/`session_id` de una sesión ad-hoc no adoptada (no está en `state.json`; el skill prohíbe descubrir surfaces) → degrada a "adopta por el dashboard" + comentario vago. El camino at-adopt es inalcanzable con este diseño. El intent se reubica en **ORCH-02** (Phase 62).

- [x] **ORCH-02** *(Phase 62 — adopción inteligente desde el dashboard, supersede el at-adopt de ORCH-01)*: Al pulsar `a` sobre un surface ad-hoc, el dashboard ejecuta un paso de **derivación LLM one-shot** (`claude -p --model claude-haiku-4-5`, headless, ≤8s, **never-throws / fail-open**) que lee la **memoria del proyecto** (GSD: `.planning/PROJECT.md` + `ROADMAP.md` + `STATE.md` → alcance global; arregla el F2 de resumen estrecho) + el **primer prompt del usuario** del transcript (`~/.claude/projects/<cwd>/<sessionId>.jsonl` → intención) + `git log --oneline` (señal secundaria), y propone `{title, description}` ANTES de crear. El operador confirma (segunda `a`) → shellea `kodo adopt --title '…' --description '…'`. **Por qué el dashboard y no el orquestador:** el dashboard YA tiene las coordenadas vía `listAgentSurfaces()` (DETECT-01) — disuelve el bloqueo de ORCH-01. **Invariantes:** suelo determinista intacto (Haiku falla → fallback a `basename(cwd)`, adopt nunca bloquea); el LLM vive SOLO en el paso de derivación; el `{title, description}` pasa igual por `sanitizeAdoptionData` (BIDIR-08); `execFile` argv → inyección shell estructuralmente inerte (T-57-01 no aplica). **Trade-off aceptado:** rompe el invariante literal "vigilante 0-token" (spawn explícito disparado por tecla, acotado) + dependencia dura del CLI `claude`. v1 no-editable (Esc → `kodo adopt --title` manual). NOTA: no hay `updateTask` para el título (FROZEN-9) → la derivación debe ocurrir ANTES del `createTask` (de ahí el spinner "derivando…").
