...
- [x] **LOG-13**: `dispatcher.js` usa `EVENTS.gsdPhaseResolved` y `EVENTS.gsdBootstrap` en vez de literales `'gsd.phase.resolved'` / `'gsd.bootstrap'`; invariante validado con grep en test source-hygiene.
- [x] **LOG-14**: `markSessionStatus` se invoca en `verify.js` cuando `verdict.action === 'pass'` y la transición Plane → Review es OK; emite `state.transition` con `from`/`to` reales (documentados como contrato del evento).
- [x] **LOG-15**: `markSessionStatus` se invoca en `stop.js` cuando se libera el lock per-repo; emite `state.transition` al estado terminal de la sesión, sin romper el flujo de release del lock.
