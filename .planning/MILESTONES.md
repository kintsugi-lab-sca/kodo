...
## v0.19 Inbox de capturas + fix stealLock + saneo de deuda (Shipped: 2026-07-28)

**Phases completed:** 4 phases (82-85), 17 plans, 35 tasks
**Git:** 137 commits desde v0.18 · 116 ficheros, +26.588/−1.397 (src+test+bin: 25 ficheros, +6.918/−119) · 2026-07-24 → 2026-07-28
**Cierre:** verified closeout — 15/15 requirements Complete · 4/4 fases `phase_complete` + `verification_status: passed` · audit de artefactos abiertos limpio · suite 2364 → **2590 tests (2589 pass / 0 fail / 1 skipped)**

**Key accomplishments:**

- **Carrera de `stealLock` cerrada por construcción (Phase 82):** el move-aside `renameSync(lockPath→aside)` se elimina del todo — la propiedad del lock pasa solo por `renameSync(tmp→lockPath)` y el steal se serializa con un steal-guard `O_EXCL` publicado atómicamente vía `linkSync` (el `writeFileSync({flag:'wx'})` del primer intento reabría la ventana briefly-empty *dentro* del guard, y se detectó con doble adquisición reproducible). Con N≥2 stealers del mismo lock muerto adquiere exactamente uno, aseverado sobre la cardinalidad y no sobre qué stealer gana. Evidencia determinista: el harness `gsd-lock-race` byte-idéntico (sin debilitar el assert, sin retries, sin subir timeouts — constraint heredado de DEBT-04) pasa 100/100 bajo carga paralela 4× frente al ~48% del repro original, +300 iteraciones sobre el rework. R-81-01 saldada = fix real; debug session `gsd-lock-race-cr01` movida a `resolved/`.
- **Núcleo del inbox — `src/inbox/store.js` (Phase 83):** codec markdown byte-exacto, parser anclado a la cola resistente a forgeries, reader never-throws, append `O_APPEND` con fail-open, y marcado RMW bajo `withFileLock` (`src/session/state-lock.js`, nunca `src/gsd/lock.js`) con publicación unique-tmp + rename que preserva byte a byte toda línea ajena. `kodo capture` y `kodo inbox route|discard` quedan como thin handlers: gate de texto vacío en exit 2, `--json` byte-determinista que jamás pasa por el formatter, y los cuatro `reason` del store mapeados a exit codes (D-13), con dos gates source-hygiene anclados al patrón de import.
- **La concurrencia del inbox se decidió con evidencia, no con confianza (Phase 83, plans 03-06):** el escenario mixto D-21 (6 capturas concurrentes durante un marcado con la ventana lectura→rename sostenida) destapó que el riesgo residual de D-03 era **total** — 0 de 6 supervivientes, el `rename` del marcado borraba cada captura que había hecho fail-open. El arreglo definitivo no es un umbral temporal sino un **guard compare-and-swap dentro del lock** (baseline de bytes de la LECTURA + inodo del destino, comprobado justo antes del `renameSync`, reintento acotado a 5, `concurrent-write` ruidoso que deja el fichero intacto); la publicación deja además de destruir symlink y permisos del operador. La suite incorpora el escenario que destruía 6/6 y se pone ROJA en cuanto el guard se revierte a mano, más un guard de cobertura cross-proceso que forzó descubrir que la colisión marcado↔captura dependía del scheduler.
- **Superficies de captura (Phase 84):** `/kodo-capture` mid-session con contrato verificable — el test extrae la cadena de comando del propio markdown, la congela contra un argv canónico y la ejecuta por dos vías, heredando la byte-identidad de CAPT-02 del writer único de Phase 83 en vez de reimplementarla. `kodo skill sync` pasa de single-skill a recorrer una **allowlist congelada** que es el control de acceso que impide que las skills de trabajo local del repo acaben en el HOME de nadie (resiliencia por skill, `--json` aditivo de orden fijo). Y un leaf de filesystem never-throws pinta el conteo de capturas abiertas en la cabecera del dashboard — invisible cuando es 0, cero endpoints nuevos.
- **Deuda de v0.18 saldada con mordida demostrada (Phase 85, DEBT-05..07):** el typedef `TaskHandoff` vuelve a describir el merge tres-estados que realmente se ejecuta y `deriveAnyNext` **delega** en `nextCell` (la incoherencia deja de ser posible por construcción, no por disciplina). El piggyback del sidebar deja de fundir «nada que arreglar» con «cmux caído»: emite el conteo de fallos por stderr con test que lo dictó en rojo. Los 3 warnings de 80-REVIEW se **resolvieron, ninguno se re-aceptó**, y el guard LOG-12 pasó de decorativo a tener mordida verificada sobre `import()` dinámico (violación inyectada → RC=1 → revert).
- **Nyquist retroactivo citation-based sobre 6 fases (Phase 85, NYQ-01/02):** 79/80/81 y 69/71/72 pasan a `status: validated` + flag Nyquist en **true**, con las 43 filas de sus Per-Task Verification Maps citando fichero + sección + conteo, y 9 items no automatizables contabilizados como Manual-Only con su razón. Cero tests generados y cero re-corridas de suite (D-12); los `MILESTONE-AUDIT.md` de v0.16 y v0.18 quedan intactos (D-15). El `skipped` del UAT de 71 (GitHub real) se citó **como skip**, jamás doblado a pass — y el bookkeeping final escribe en la propia celda que **contabilizar no es resolver**, para que el siguiente audit no lo lea como cierre.

**Deuda diferida con trigger (trazada, no silenciada):** **R-82-01** — carrera de *segundo orden* en `stealLock` con holder **VIVO** (el branch PRESENT hace `renameSync` incondicional y los creadores Case-1 no pasan por el guard), fuera del criterio literal LOCK-01 (holder muerto, cerrado y probado) e invisible al harness; fix por construcción exige rediseñar el primitivo → **decisión de mantenedor pendiente**, sin parche apresurado. · **D-08b**: el auto-sync del orquestador sigue distribuyendo solo `kodo-orchestrate`; quien nunca ejecute `kodo skill sync` no recibe `/kodo-capture`. · **format-isolation transitivo + OQ-1**: evaluado y diferido explícitamente por la Phase 85 (D-18) — se corrigen juntos, radio sin medir. · **Evidencia en vivo** de 79/80 y **backstop GitHub real** de 71: mejor contabilizadas, siguen abiertas con su trigger original. Detalle por ítem en STATE.md §Deferred Items y en los `deferred-items.md` de las fases 84/85.

**Nota de cierre:** milestone cerrado sin `v0.19-MILESTONE-AUDIT.md` (como v0.12 y v0.14) — las 4 fases estaban `passed` en su propia verificación y la cobertura de requirements era 15/15 con traceability completa.
