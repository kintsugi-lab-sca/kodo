...
**v0.19 Inbox de capturas + fix stealLock + saneo de deuda** — shipped 2026-07-28 (4 phases 82-85, 17 plans, 35 tasks; verified closeout — 15/15 reqs · 4/4 fases `passed` · audit de artefactos abiertos limpio; suite 2364 → 2590; 137 commits en 4 días). **Phase 82** — carrera de `stealLock` cerrada **por construcción**: move-aside eliminado + steal-guard `O_EXCL` publicado atómicamente vía `linkSync` (el `writeFileSync({flag:'wx'})` del primer intento reabría la ventana *dentro* del guard); harness byte-idéntico 100/100 bajo carga 4× frente al ~48% del repro. R-81-01 saldada = fix; hallazgo de 2º orden trazado como **R-82-01**. **Phase 83** (7 plans) — buffer de captura global: `kodo capture` con append `O_APPEND` atómico y `kodo inbox` que marca `enrutada`/`descartada` sin borrar jamás; el invariante de concurrencia falló en la primera verificación y se cerró con un **guard compare-and-swap dentro del lock** anclado al estado del fichero (bytes + `ino`), tras revertir el umbral temporal por ser el enmascaramiento que DEBT-04 prohíbe. **Phase 84** — `/kodo-capture` mid-session (writer único, argv congelado), `kodo skill sync` multi-skill con allowlist literal como control de acceso, y conteo ambient en el dashboard. **Phase 85** — deuda doc de v0.18 saldada (DEBT-05/06), los 3 warnings de 80-REVIEW resueltos sin re-aceptar ninguno (DEBT-07), y 6 fases retro-validadas citation-based (NYQ-01/02) con cero tests generados.

- Roadmap archive: `milestones/v0.19-ROADMAP.md`
- Requirements archive: `milestones/v0.19-REQUIREMENTS.md`
- Audit: no generado (4/4 fases `passed` + 15/15 reqs; precedente v0.12/v0.14)
- Phases: `milestones/v0.19-phases/`

(Anterior: **v0.18 Higiene del sidebar de cmux**, shipped 2026-07-24 — archivos en `milestones/v0.18-*`.)
...
- **Bandeja del orquestador (v0.20, KODO-53):** los eventos del ciclo de vida hacia el orquestador (fin de sesión, sesión lanzada) se PERSISTEN en `state.orchestrator_inbox` y se leen en la ronda; el carril de TECLADO (`cmux send`) queda reservado a un aviso de UNA línea que solo sale si el orquestador está **idle** (`isOrchestratorIdle` es FAIL-CLOSED: pantalla vacía, ilegible o con borrador ⇒ no se teclea) y con debounce de ~30 s anclado en `notified_at`. El nudge de «Nueva sesión lanzada» NO vuelve al teclado en ningún modo. `ack` transiciona (`seen`+`seen_at`), jamás borra; la eviction FIFO (`SEEN_CAP` 50) solo toca las YA VISTAS. Todo escritor pasa por `withStateLock`, y `hooks/session-end.js` / `session/manager.js` exponen los seams `enqueueOrchestratorEventFn` / `maybeNotifyOrchestratorFn` — sin ellos la suite escribe en el `~/.kodo/state.json` real y puede teclear en el terminal del operador (medido: 59 eventos falsos en la primera pasada sin stubs).
