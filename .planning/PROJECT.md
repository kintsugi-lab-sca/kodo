...
**In scope for v0.6:**
- [~] **Worktree always-on**: runtime wiring completo en Phase 18 (WT-01/02/03 — `--worktree <sessionId>` en `launchWorkItem`, persistencia PRE-spawn, lock per-repo invariante). Cleanup pendiente en Phase 19 (WT-04 stop hook `git worktree remove` fail-open, WT-05 auto-commit cwd, WT-06 `kodo gsd verify` path).
...
---
*Last updated: 2026-05-12 — Phase 18 (Worktree Runtime Wiring) complete: WT-01/02/03 satisfied, runtime wiring de `--worktree <sessionId>` activo en todas las sesiones, lock per-repo invariante WT-03 cross-source verified (0 matches), 547 tests / 546 pass. CR-02 (SessionRecord huérfano) diferido a Phase 19 cleanup. Próximo: Phase 19 — worktree cleanup & integration (WT-04/05/06).*
