...
**Next:** v0.9 — kodo TUI sesiones en vivo (en curso: Phase 35 datos completa, 6/16 requirements TUI)
...
---
*Last updated: 2026-05-27 — Phase 35 (Datos — cliente HTTP + polling) completa: `fetchStatus` puro never-throws (D-07), hook `usePoll` self-scheduling (single-flight + backoff + timeout + teardown), status line viva en `App.js` (keep-last-good + 3 estados) y WR-01 cerrado vía `resolveBaseUrl`; TUI-05/TUI-06 validados (VERIFICATION 14/14, suite 915 pass). Siguiente: Phase 36 (Tabla viva — render + selección + filtros).*
