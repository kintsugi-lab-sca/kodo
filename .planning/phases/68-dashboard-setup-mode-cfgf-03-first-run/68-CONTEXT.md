...
- **D-08:** **Aviso de reinicio honesto**, sin auto-restart: al completar el setup el dashboard muestra "config guardada — reinicia kodo (`kodo up`) para aplicar". Es el literal del SC#4 ("aviso de reinicio honesto, sin hot-reload, coherente con v0.14"). Simple, never-throws, sin orquestar restart del daemon ni tensar con el modelo daemon persistente LOCKED. El operador re-ejecuta `kodo up`, que ahora encuentra config completa y arranca normal (D-02 se auto-resuelve en el 2º arranque).
