# Requirements Archive: v0.5 CLI Polish & v0.3 Debt Cleanup
...
- [x] **DX-01**: `kodo logs` colorea cada nivel (debug=gris, info=cyan, warn=amarillo, error=rojo) con TTY detection y respeto a `NO_COLOR` / `FORCE_COLOR`.
- [x] **DX-02**: `kodo logs` reformatea stderr con columnas alineadas (`timestamp · level · component · message`); `--json` no se ve afectado.
- [x] **DX-03**: `kodo gsd inspect <task-id>` presenta el verdict del resolver con símbolos `✓`/`✗` por sección y exit code visible al final.
- [x] **DX-04**: `kodo gsd verify <session-id>` colorea pass / soft-fail / hard-fail (verde / amarillo / rojo) y muestra resumen del comentario Plane.
- [x] **DX-05**: `kodo check` colorea OK/FAIL en su tabla sin cargar `src/logger.js` (preserva el guard LOG-12 de bajo coste).
- [x] **DX-06**: Helper `src/cli/format.js` centraliza color/format con TTY detection — invariante: `--json` produce bytes idénticos al output sin TTY (validado por test source-hygiene + golden bytes).
- [x] **DX-07**: `picocolors` añadido como dependencia en `package.json` y reflejado en el lockfile; documentado en PROJECT.md (primer aumento de deps externas desde v0.2 / commander).
...
|-------------|-------|--------|
| DX-06 | Phase 14 | Complete |
| DX-07 | Phase 14 | Complete |
...
*Last updated: 2026-05-11 — All 13 v0.5 requirements satisfied (checkbox drift corrected at milestone close)*
