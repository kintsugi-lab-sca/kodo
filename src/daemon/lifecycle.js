 ...
 *   2. Pre-flight: readPidFile(name) vivo → already-running (no spawn); stale (file
 *      presente, proceso muerto) → removePidFile(name) y procede (polling.js:259-268).
 *   3. Detached spawn `execPath + [KODO_BIN, ...argv]` + child.unref() (polling.js:283-303).
 *   4. Bounded wait: poll readPidFile(name)+isPidAlive hasta `waitMs` (polling.js:315-323).
 *
 ...
 * @returns {Promise<{ ok: boolean, alreadyRunning?: boolean, started?: boolean,
 *   timedOut?: boolean, unsupported?: boolean, pid?: number, message?: string }>}
 */
export async function startDaemon(name, argv, deps = {}) {
  ...
  // 2) Pre-flight (Pitfall #3): check en el padre ANTES del spawn.
  ...
  // 3) Detached spawn con argv absoluto (T-65-10 EOP: cero PATH lookup, argv array form).
  const child = spawnFn(
    execPath,
    [kodoBin, ...argv],
    {
  ...
  // 4) Bounded wait (polling.js:315-323): poll PID file + isPidAlive hasta el deadline.
