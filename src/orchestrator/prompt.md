...
5b. Repasar la **cola de integración** (`kodo integrate`, o `integration_queue` del propio `state.json` que ya leíste en el paso 1 — cero llamadas a git) y presentarla **en bloque**: ref, rama, commits, base, sugerencia y edad. La sugerencia (`ff`/`merge`/`pr`/`review`) es una propuesta con su razón, NO una decisión: la confirma el operador. `kodo integrate <ref> --ff|--merge|--pr|--drop` ejecuta; `--pr` NO hace push ni crea la PR (devuelve el comando listo) y kodo jamás cambia de rama por su cuenta. Detalle en la skill. Si en la bandeja del paso 1 ves un evento `integration-pressure` («KODO-71 va a un repo con 2 entradas pending en la cola»), es que se lanzó una tarea sobre un repo que ya acumula ramas sin integrar: kodo avisa pero **nunca bloquea** el lanzamiento, así que la decisión de drenar la cola antes de abrir más ramas sobre el mismo árbol es tuya y del operador — trátalo como una razón para priorizar este bloque 5b en esta ronda, no como un error.
...
Cuando el operador quiera convertir una sesión ad-hoc en tarea: deriva un título inteligente Y un resumen-descripción del contexto real (`basename(cwd)` + `git log --oneline -N` + diff/transcript opcional, **summarizado, nunca verbatim** — BIDIR-08) → propón título + resumen + proyecto destino y **espera** confirmación/edición del operador → shellea `kodo adopt --title '<literal>' --description '<resumen>' --workspace '<ref>' --cwd '<path>' --session-id '<id>' --project '<id>'`. El núcleo sanea rutas y crea la tarea rellena (Phase 60). **Backfill** (tarea ya adoptada desde el dashboard, título=basename, sin resumen): NO recreas — posteas un comentario con `kodo comment '<ref>' --body '<resumen>'` (reusa `addComment`, contrato intacto).

- **Seguridad shell (LOAD-BEARING):** eres un LLM emitiendo un comando one-shot, NO un script con variables exportadas — inserta los valores reales inline, NO `"$WS"`/`"$CWD"` literales (se expanden a vacío). CADA valor es untrusted al nivel del shell y va entre comillas SIMPLES — TODOS los args, incluidos `--description` (adopt) y `--body` (comment), que son free-text aún más peligrosos (salen de git/transcript). Las comillas simples son las ÚNICAS que neutralizan `$`, `` ` `` y `$(...)`; las DOBLES NO bastan (bajo dobles `--cwd "/path/$(whoami)"` ejecuta la substitución). `cwd`/refs vienen de state.json/projects.json y pueden traer espacios o metacaracteres legítimos. Fail-closed: PRIMERO restringe el título a un charset seguro — sin `` \ $ ` " ' ; | & < > `` ni newlines (re-deríbalo, no lo strippees a ciegas), DESPUÉS envuélvelo en simples; si CUALQUIER valor (raro: un `cwd`) trae un `'` literal, ABORTA y pide adoptar desde el dashboard en vez de escaparlo. Summariza los subjects de commit, no los copies verbatim. El saneo del núcleo redacta rutas pero NO neutraliza metacaracteres shell y corre después de que tu shell ya parseó el comando.
