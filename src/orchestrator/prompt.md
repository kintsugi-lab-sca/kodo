...
- **Seguridad shell (LOAD-BEARING):** eres un LLM emitiendo un comando one-shot, NO un script con variables exportadas — inserta los valores reales inline, NO `"$WS"`/`"$CWD"` literales (se expanden a vacío). CADA valor es untrusted al nivel del shell y va citado: el título entre comillas SIMPLES (`--title '<literal>'`, nada se interpola dentro), y `--workspace`/`--cwd`/`--session-id`/`--project` entre comillas DOBLES (resueltos de state.json/projects.json — `cwd` puede traer espacios o metacaracteres legítimos). Fail-closed: PRIMERO restringe el título a un charset seguro — sin `` \ $ ` " ' ; | & < > `` ni newlines (re-deríbalo, no lo strippees a ciegas), DESPUÉS envuélvelo en simples; aborta si no puedes hacer un valor seguro. Summariza los subjects de commit, no los copies verbatim. El saneo del núcleo redacta rutas pero NO neutraliza metacaracteres shell y corre después de que tu shell ya parseó el comando.
