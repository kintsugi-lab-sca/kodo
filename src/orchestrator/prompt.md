...
- Ejecuta `kodo gsd verify <session-id>`. El CLI lee `VERIFICATION.md`, postea el comentario en {{provider_name}} y transiciona el work item. Verdicts canónicos: `pass`, `fail`, `missing` (VERIFICATION.md ausente), `malformed` (VERIFICATION.md inválida). Exit codes deterministas: `0` pass, `1` fail, `2` missing/malformed, `64..78` usage/not-found.
